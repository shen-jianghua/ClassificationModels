...
parser.add_argument('--batch_size', type=int, default=12, help='batch_size for training')
parser.add_argument('--lr', type=float, default=0.002, help='star learning rate')
parser.add_argument('--lrf', type=float, default=0.001, help='end learning rate')
...
parser.add_argument('--data_path', type=str, default="data/flower")
parser.add_argument('--model', type=str, default="densenet", help=' select a model for training')
...
def main(args):
    ...
    if opt.tensorboard:
        ...
        log_path = os.path.join('data/results/tensorboard', args.model)
    ...
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)      # 用户自定义的学习率调整规则
    best_acc = 0.
    ...
    for epoch in range(args.epochs):
        ...
        with open(os.path.join(save_path, "DenseNet_log.txt"), 'a') as f:
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), os.path.join(save_path, "DenseNet.pth"))
